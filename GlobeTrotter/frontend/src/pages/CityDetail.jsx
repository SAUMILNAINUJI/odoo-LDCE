...
import { ArrowLeft, CalendarDays, Heart, MapPin, TrendingUp, Wallet } from 'lucide-react'
...
export default function CityDetail() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [cityResponse, activityResponse, pointsResponse, reviewsResponse, favoritesResponse] = await Promise.all([
          api.get(`/cities/${id}`),
          api.get('/activities', { params: { city_id: id } }),
          api.get(`/discovery/cities/${id}/points-of-interest`),
          api.get(`/discovery/reviews/${id}`),
          api.get('/discovery/favorites')
        ])
        ...
        setSaved(favoritesResponse.data.some(item => item.entity_type === 'city' && Number(item.entity_id) === Number(id)))
      } catch (requestError) {
  ...
  const toggleSaved = async () => {
    const { data } = await api.post('/discovery/favorites', { entity_type: 'city', entity_id: id })
    setSaved(data.saved)
  }
